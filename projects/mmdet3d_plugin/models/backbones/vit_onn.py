...
class ONNBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.down_channel = nn.Linear(dim * onn_channel_ratio, dim, 1)
        if up_channels:
            self.up_channel = nn.Linear(dim, dim * onn_channel_ratio, 1)
        else:
    ...
    def forward(self, x, H, W):
        ...
        bs, n_token2, c = x.shape
        n_token = int(np.sqrt(n_token2))
        if self.gamma_1 is None:
            # x = x + self.drop_path(self.attn(self.norm1(x), H, W))

            new_x = self.norm1(x)
            new_x = new_x.permute(0, 2, 1).contiguous().view(bs, c, n_token, n_token)
            new_x = self.attn(new_x)
            new_x = new_x.permute(0, 2, 3, 1).view(bs, -1, c)
            x = x + self.drop_path(new_x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), H, W))
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        if self.up_channel is not None:
            x = self.up_channel(x)
        return x
...
class ONNViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if len(onn_depth) != 0:
            embed_dim = int(embed_dim // onn_channel_ratio)
        if merge_to_one:
            j = onn_depth[0]
            onn_block = ONNBlock(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                ...
                window_size=(14, 14) if ((j + 1) % interval != 0) else self.patch_embed.patch_shape, window=((j + 1) % interval != 0),
                onn_cfg=onn_cfg,
                up_channels=up_channels,
                onn_channel_ratio=onn_channel_ratio,
                use_abs=use_abs,
                use_bias=use_bias,
                use_square=use_square)
            self.blocks.append(onn_block)
        else:
            for _, j in enumerate(onn_depth):
                onn_block = ONNBlock(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j], norm_layer=norm_layer,
                    init_values=init_values, 
        ...
        ## fool code
        if up_channels:
            embed_dim = embed_dim * onn_channel_ratio
    ...
    def forward_features(self, x, return_backbone):
        B, C, H, W = x.shape
        ...
        if return_backbone:
            return tuple(features), xp

        return tuple(features)

    def forward(self, x, return_backbone=False):
        x = self.forward_features(x, return_backbone)
        if return_backbone:
            return x[0], x[1]
        return x
