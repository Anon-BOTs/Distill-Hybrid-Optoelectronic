...
class UVTRGP(MVXTwoStageDetector):
    ...
    def forward_pts_train(
        ...
        if self.with_depth_head and hasattr(self.pts_bbox_head.view_trans, "loss"):
            losses.update(
                self.pts_bbox_head.view_trans.loss(img_depth, points, cur_img, img_metas)
            )
        return losses
    ...
    def forward_train(self, points=None, img_metas=None, img=None):
        ...
        pts_feats, img_feats, img_depth = self.extract_feat(
            points=points, img=cur_img, img_metas=img_metas
        )
        losses = dict()
    ...
    def simple_test(self, img_metas, points=None, img=None):
        ...
        batch_rays = self.pts_bbox_head.sample_rays_test_v2(points, img, img_metas)
        results = self.pts_bbox_head(
            pts_feats, img_feats, img_metas, img_depth, batch_rays
        )
        with open("outputs_gp/{}.pkl".format(img_metas[0]["sample_idx"]), "wb") as f:
            H, W = img_metas[0]["img_shape"][0][0], img_metas[0]["img_shape"][0][1]
            num_cam = len(img_metas[0]["img_shape"])
            pickle.dump(
                {
                    "render_rgb" : results["img_rgb"].detach().cpu().numpy(),
                    "render_depth" : results['img_depth'].detach().cpu().numpy(),
                    "rgb": batch_rays[0]["img_rgb"].detach().cpu().numpy(),
                    "occ": results["voxel_opacity"].detach().cpu().numpy()
                },
                f,
            )
