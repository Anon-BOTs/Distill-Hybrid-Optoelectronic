...
class Petr3D(MVXTwoStageDetector):
    ...
    def show_results(self, data, result, **kwargs):
        """
        data,
        result,
        out_dir=out_dir,
        ...
        import numpy as np
        import random
        filenames = data.get('img_metas')[0]._data[0][0]['filename']
        tasks = [  # 第一个必须是class
            dict(
                task_name = 'class',
                num_out = 10,
        ...
        lidar2cam = data['extrinsics'][0]._data[0][0].cpu().numpy()
        cam2lidar = np.linalg.inv(lidar2cam)
        intrinsics = data['intrinsics'][0]._data[0][0][:, :3, :3].cpu().numpy()
        all_rots = cam2lidar[:, :3, :3]
        all_trans = cam2lidar[:, :3, 3]

        ## vis 2d
        for img_i, res in enumerate(result):
            ...
            # img = np.array(data.get('img_metas')[0]._data[0][0]['ori_img'][img_i])
            ...
            cv2.imwrite(f'/root/tmp/vis_2d/{basename}',img)
        ...
        # mask = result[0]['pts_bbox']['scores_3d'].numpy() > 0.2
        # bbox = result[0]['pts_bbox']['boxes_3d'].tensor.numpy()[mask]
        # bbox[:, 2] += bbox[:, 5] / 2
        # pred_dict = {
        #     'box' : bbox,
        #     'class' : result[0]['pts_bbox']['labels_3d'].numpy()[mask]
        # } 
