...
class PETRHeadDNV2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadDNV2):
        ...
        super(PETRHeadDNV2, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_raydn(self, batch_size, reference_points, img_metas, data):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadDNV2:
            convert_dict = {
            ...
            for k in state_dict_keys:
                for ori_key, convert_key in convert_dict.items():
                    if ori_key in k and 'bbox_head' in k:
    ...
    def forward(self, memory_center, img_metas, topk_indexes,  **data):
        ...
        if 'outs_roi' in data and 'memory' in data['outs_roi']:
            outs_roi = data['outs_roi']
            if 'dn_meta_2d' in outs_roi and outs_roi['dn_meta_2d'] is not None:
                dn_meta = outs_roi['dn_meta_2d']
                memory_center_ori = outs_roi['enc_bbox_preds'][:, :, dn_meta['pad_size']:, :]
                memory = outs_roi['memory'][:, :, dn_meta['pad_size']:, :]
            else:
                memory_center_ori = outs_roi['enc_bbox_preds']
                memory = outs_roi['memory']
            ...
            if not self.use_3d_pe:
                if self.detach:
                    memory = memory.detach().reshape(B, -1, C)
                    memory_center = memory_center_ori.detach().clone()
                else:
                    memory = memory.reshape(B, -1, C)
                    memory_center = memory_center_ori.detach().clone()
                memory_center = memory_center[-1]
                memory_center = memory_center[..., :2]
                memory_center = memory_center.unsqueeze(1)
        ...
        if self.use_ray_dn:
            reference_points, attn_mask, mask_dict = self.prepare_for_raydn(B, reference_points, img_metas, data)
        else:
        ...
        # self.show_results(data, outs, img_metas)
        return outs
    ...
    def show_results(self, data, result, img_metas, use_gt=False, **kwargs):
        """
        data,
        result,
        out_dir=out_dir,
        ...
        cams = ['CAM_FRONT_RIGHT', 'CAM_FRONT', 'CAM_FRONT_LEFT', 'CAM_BACK_LEFT', 'CAM_BACK',  'CAM_BACK_RIGHT']
        intrinsics = data['intrinsics'][0].cpu().numpy()
        ...
        result = self.get_bboxes(result, img_metas)
        ...
        for img_i, cam_name in enumerate(cams):
            ...
            img = np.array(img_metas[0]['ori_img'][index])
            all_imgs.append(img)
        ...
        if not use_gt:
            ...
            bbox = result[0][0].tensor.detach().cpu().numpy()[mask]
            bbox[:, 2] += bbox[:, 5] / 2
            pred_dict = {
                'box' : bbox,
                'class' : result[0][2].detach().cpu().numpy()[mask]
            } 
        else:
            bbox = img_metas[0]['gt_bboxes_3d']._data.tensor.numpy()
            bbox[:, 2] += bbox[:, 5] / 2
            pred_dict = {
                'box' : bbox,
                'class' : img_metas[0]['gt_labels_3d']._data.numpy()
            } 
        ...
        cv2.imwrite(os.path.join(show_dir, 'head_' + sample_idx + '.jpg'), img)
