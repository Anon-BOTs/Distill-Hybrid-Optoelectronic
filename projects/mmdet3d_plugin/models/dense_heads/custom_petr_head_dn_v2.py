...
class CustomPETRHeadDNV2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is CustomPETRHeadDNV2):
        ...
        super(CustomPETRHeadDNV2, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is CustomPETRHeadDNV2:
            convert_dict = {
    ...
    def forward(self, memory_center, img_metas, topk_indexes,  **data):
        ...
        if 'outs_roi' in data and 'memory' in data['outs_roi']:
            ...
            if outs_roi['memory'].dim() == 4:
                memory = outs_roi['memory'][-1]
            else:
                memory = outs_roi['memory']
            BN, _, C = memory.shape
            if outs_roi.get('use_2d_box', True):
                if self.detach:
                    memory = memory.detach().reshape(B, -1, C)
                    memory_center_all = outs_roi['enc_bbox_preds'].detach().clone()
                else:
                    memory = memory.reshape(B, -1, C)
                    memory_center_all = outs_roi['enc_bbox_preds'].detach().clone()
                memory_center_all = memory_center_all[-1]
                memory_center_all = memory_center_all[..., :2]
                memory_center = memory_center_all.unsqueeze(1)
